...
import com.eggze.spreadit.tcp.requests.StatsRequest;
 ...
 * @since 0.1
 */
public class StatsPeriodicWorker extends BaseWorker {
    public StatsPeriodicWorker(Context appContext, WorkerParameters workerParams) {
        super(appContext, workerParams);
    }

    public final static String TAG = BaseWorker.PREFIX + "STATS";
    public final static String TAG_ONE_TIME = BaseWorker.PREFIX + "STATS_ONE_TIME";


    @Keep
    private final static long REPEAT_INTERVAL = 24;
    @Keep
    private final static long INITIAL_DELAY_INTERVAL = 5L;
    @Keep
    private final static long FLEX_INTERVAL = 2;
    @Keep
    private final static long BACKOFF_INTERVAL = 2;

    public static void schedulePeriodic(Context context) {
        schedule(context, TAG, StatsPeriodicWorker.class, null,
                REPEAT_INTERVAL, TimeUnit.HOURS,
                FLEX_INTERVAL, TimeUnit.HOURS,
                INITIAL_DELAY_INTERVAL, TimeUnit.SECONDS,
                BackoffPolicy.EXPONENTIAL, BACKOFF_INTERVAL, TimeUnit.MINUTES);
    }

    public static void scheduleOneTime(Context context) {
        scheduleOneTime(context, TAG_ONE_TIME, StatsPeriodicWorker.class,
    ...
    private void send(final User user, final ResponseListener responseListener) {
        Request request = new StatsRequest(user, responseListener);
        request.send();
    }
}
