 ...
 * @since 0.1
 */
public class UserResultRequest extends Request {

    public UserResultRequest(User user, UserResult userResult, ResponseListener responseListener) {
        super(responseListener);
        UserResultPacket utp = new UserResultPacket(networkManager.getClient(), networkManager.getClient().getSocket());
        utp.setPacketIndex((short) userResult.getId());
        utp.setUserUUID(user.getUuid());
        utp.setResultUUID(userResult.getUuid());
        setRequestPacket(utp);
    }
}
