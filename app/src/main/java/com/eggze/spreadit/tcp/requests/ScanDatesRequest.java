 ...
 * @since 0.1
 */
public class ScanDatesRequest extends Request {

    public ScanDatesRequest(User user, ResponseListener responseListener) {
        super(responseListener);
        ReqDatesPacket rdp = new ReqDatesPacket(networkManager.getClient(), networkManager.getClient().getSocket());
        rdp.setPacketIndex((short) 1);
        rdp.setUserUUID(user.getUuid());
        setRequestPacket(rdp);
    }
}