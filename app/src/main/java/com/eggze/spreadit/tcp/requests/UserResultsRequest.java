 ...
 * @since 0.1
 */
public class UserResultsRequest extends Request {

    public UserResultsRequest(User user, ResponseListener responseListener) {
        super(responseListener);
        ReqResultsPacket rrp = new ReqResultsPacket(networkManager.getClient(), networkManager.getClient().getSocket());
        rrp.setPacketIndex((short) 1);
        rrp.setUserUUID(user.getUuid());
        setRequestPacket(rrp);
    }
}