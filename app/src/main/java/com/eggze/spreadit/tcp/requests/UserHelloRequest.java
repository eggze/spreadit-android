 ...
 * @since 0.1
 */
public class UserHelloRequest extends Request {

    public UserHelloRequest(User user, ResponseListener responseListener) {
        super(responseListener);
        UserHelloPacket nup = new UserHelloPacket(networkManager.getClient(), networkManager.getClient().getSocket());
        nup.setUserUUID(user.getUuid());
        nup.setPacketIndex((short) user.getId());
        setRequestPacket(nup);
    }
}
