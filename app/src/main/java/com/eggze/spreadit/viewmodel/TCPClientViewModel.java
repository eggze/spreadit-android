/**
 * This file is part of spreadit-android. Copyright (C) 2020 eggze Technik GmbH
 *
 * spreadit-android is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * spreadit-android is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program. If not, see <http://www.gnu.org/licenses/>.
 */
package com.eggze.spreadit.viewmodel;

import android.app.Application;

import androidx.annotation.NonNull;

import com.eggze.spreadit.tcp.TCPStatusListener;
import com.eggze.spreadit.work.HelloPeriodicWorker;

/**
 *
 * @author D. Kalyvas <dimitris at kalyvasd.eu>
 * @version 0.1
 * @since 0.1
 */
public class TCPClientViewModel extends BaseViewModel implements TCPStatusListener {
    public TCPClientViewModel(@NonNull Application application) {
        super(application);
    }

    private int status = -1;

    @Override
    public void statusChanged(int status) {
        if (this.status != status) {
            this.status = status;
            switch (status) {
                case TCPStatusListener.STATUS_CONNECTED:
                    HelloPeriodicWorker.scheduleOneTime(getApplication());
                    break;
                case TCPStatusListener.STATUS_CONN_FAILED:
                case TCPStatusListener.STATUS_DISCONNECTED:
                case TCPStatusListener.STATUS_CONNECTING:
                    break;
            }
        }
    }

    public void connectClient() {
        if (networkManager != null) {
            networkManager.registerStatusListener(this);
        }
    }

    public void disconnectClient() {
        if (networkManager != null) {
            networkManager.unregisterStatusListener(this);
        }
    }
}
