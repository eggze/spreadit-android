...
public final class SecureSocket implements SocketIF {
	...
	private static final Logger logger = LoggerHandler
			.getLogger(SecureSocket.class.getName());
	...
	private boolean first_not_handshaking = true;
	...
	public void processHandshake() throws IOException {
		...
		switch (status) {
		case NEED_TASK:
			logger.log(Level.FINEST, "{0} NEED_TASK", sc.socket()
		...
		case NEED_UNWRAP:
			logger.log(Level.FINEST, "{0} NEED_UNWRAP", sc.socket()
					.getRemoteSocketAddress());
			// Don’t read if inbound is already closed
			logger.log(Level.FINEST,
					"encryptedIn before read pos {0} lim {1} cap {2}",
					new Object[] { encryptedIn.position(), encryptedIn.limit(),
							encryptedIn.capacity() });
			count = engine.isInboundDone() ? -1 : sc.read(encryptedIn);
			logger.log(Level.FINEST, "Read {0} bytes", count);
			logger.log(Level.FINEST,
					"encryptedIn after read pos {0} lim {1} cap {2}",
					new Object[] { encryptedIn.position(), encryptedIn.limit(),
							encryptedIn.capacity() });
			encryptedIn.flip();
			logger.log(Level.FINEST,
					"encryptedIn after flip pos {0} lim {1} cap {2}",
					new Object[] { encryptedIn.position(), encryptedIn.limit(),
							encryptedIn.capacity() });
			try {
				...
				logger.log(
						Level.FINEST,
						"encryptedIn after compact pos {0} lim {1} cap {2}",
						new Object[] { encryptedIn.position(),
								encryptedIn.limit(), encryptedIn.capacity() });
			...
			break;
		case NEED_WRAP:
			logger.log(Level.FINEST, "{0} NEED_WRAP", sc.socket()
					.getRemoteSocketAddress());
			decryptedOut.flip();
			...
			break;
		case FINISHED:
			logger.log(Level.FINEST, "{0} FINISHED", sc.socket()
					.getRemoteSocketAddress());
		...
		case NOT_HANDSHAKING:
			logger.log(Level.FINEST, "{0} NOT_HANDSHAKING", sc.socket()
					.getRemoteSocketAddress());
			if (first_not_handshaking) {
				first_not_handshaking = false;
				handshakePending = false;
				hsListener.handshakeComplete(this);
				break;
			} else {
		...
		case BUFFER_UNDERFLOW:
			...
			// the handshake
			logger.log(Level.FINEST, "{0} BUFFER_UNDERFLOW", sc.socket()
					.getRemoteSocketAddress());
			return;
		case BUFFER_OVERFLOW:
			logger.log(Level.FINEST, "{0} BUFFER_OVERFLOW", sc.socket()
					.getRemoteSocketAddress());
			// Return as the encrypted buffer has not been cleared yet
			return;
		case CLOSED:
			logger.log(Level.FINEST, "{0} CLOSED", sc.socket()
					.getRemoteSocketAddress());
			...
			return;
		case OK:
			logger.log(Level.FINEST, "{0} OK", sc.socket()
					.getRemoteSocketAddress());
    ...
    public int read(ByteBuffer buffer) throws IOException {
        ...
        //logger.log(Level.FINEST, "encryptedIn before read pos {0} lim {1} cap {2}",
        //        new Object[]{encryptedIn.position(), encryptedIn.limit(), encryptedIn.capacity()});
        ...
        logger.log(Level.FINEST, "encryptedIn after read pos {0} lim {1} cap {2}",
                new Object[]{encryptedIn.position(), encryptedIn.limit(), encryptedIn.capacity()});
        // It can be the case that multiple unwraps are needed to get all the data out.
        // TODO: This is potentially also true while wrapping (also for SECURENIO JAVA)
        while(encryptedIn.position() > 0){
        	pos = 0;
        	decryptedIn.clear();
        	// Flip the encrypted data
        	encryptedIn.flip();
        	logger.log(Level.FINEST, "encryptedIn after flip pos {0} lim {1} cap {2}",
                    new Object[]{encryptedIn.position(), encryptedIn.limit(), encryptedIn.capacity()});
        	// Unwrap the data just read
        	result = engine.unwrap(encryptedIn, decryptedIn);
        	// Prepare for subsequent reads or unwraps
        	encryptedIn.compact();
        	logger.log(Level.FINEST, "decryptedIn after unwrap {0} lim {1} cap {2}",
                    new Object[]{decryptedIn.position(), decryptedIn.limit(), decryptedIn.capacity()});
        	logger.log(Level.FINEST, "encryptedIn after compact pos {0} lim {1} cap {2}",
                    new Object[]{encryptedIn.position(), encryptedIn.limit(), encryptedIn.capacity()});
        	// Copy any decrypted data to the buffer given to us
        	for (int i = pos; i < decryptedIn.position(); i++) {
        		buffer.put(decryptedIn.get(i));
        	}
        	tmpCnt = tmpCnt + decryptedIn.position()-pos;
        	if(result.getStatus() != SSLEngineResult.Status.OK){
        		// Underflow, overflow, or closed, handle it below
        		// Need to break from the loop, before we do,
        		// reset the decryptedIn buffer JUST IN CASE
        		// more reads follow for whatever reason
        		//pos = 0;
        		//decryptedIn.clear();
        		break;
        	}
        ...
        switch (result.getStatus()) {
            case BUFFER_UNDERFLOW:
                ...
                // Return tmpCnt as we might have read some bytes previously EVEN THOUGH
                // we received a buffer underflow from the SSLENGINE, TODO SECURENIO JAVA
                return tmpCnt;
            case BUFFER_OVERFLOW:
                if (!timeout.hasExpired()) {
                    // cancel any previous timeout
	...
	private int flush() throws IOException {
		...
		logger.log(Level.FINEST, "{0} Flushed {1} bytes, {2} retries.",
				new Object[] { sc.socket().getRemoteSocketAddress(), countOut,
						retries <= 1 ? 0 : retries });
		return countOut;
	}
