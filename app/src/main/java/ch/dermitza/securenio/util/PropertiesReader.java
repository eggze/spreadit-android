...
public class PropertiesReader{
    ...
    // Commented due to https://issuetracker.google.com/issues/36919831
    ...
    static {
        try  {
            // Changed due to https://issuetracker.google.com/issues/36919831
        	InputStream in = PropertiesReader.class.getResourceAsStream("/assets/setup.properties");
            props.load(in);
            in.close();
            ...
            logger.log(Level.SEVERE, "NPException while reading properties, exiting", ex);
            System.exit(-1);
        }
    }

    public static byte getProtocolVersion(){
        int i = getPropAsInt("protocol.version");

        if (i < 0) {
            logger.log(Level.SEVERE,
                    "protocol.version value is invalid: {0}. Shutting down", i);
            System.exit(-1);
        }
        return (byte)i;
    }
