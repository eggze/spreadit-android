...
public abstract class AbstractSelector implements Runnable, TaskListener, HandshakeListener, TimeoutListener {
    ...
    // TODO: Android changes
    public AbstractSelector(String addressString, int port,
            AbstractPacketWorker packetWorker, boolean usingSSL,
            boolean isClient, boolean needClientAuth){
     ...
     */
    public void setupSSL(Context androidContext, char[] tsPassPhrase, char[] ksPassPhrase) {
        if (!usingSSL) {
        ...
        if (isClient || (!isClient && needClientAuth)) {
            ...
            try {
                ...
                // https://stackoverflow.com/a/33197845
                int bks_version;
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
                    bks_version = androidContext.getResources().getIdentifier(BuildConfig.truststore, "raw", androidContext.getPackageName()); //The BKS file
                } else {
                    bks_version = androidContext.getResources().getIdentifier(BuildConfig.truststoreV1, "raw", androidContext.getPackageName()); //The BKS file
                }
                is = androidContext.getResources().openRawResource(bks_version);
                //fis = new FileInputStream(trustStoreLoc);
                //fis = new FileInputStream()
                ks.load(is, tsPassPhrase);
        ...
        if (!isClient || (isClient && needClientAuth)) {
            ...
            try {
                ...
                int bks_version;
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
                    bks_version = R.raw.clientks; //The BKS file
                } else {
                    bks_version = R.raw.clientks_v1; //The BKS (v-1) file
                }

                is = androidContext.getResources().openRawResource(bks_version);
                //fis = new FileInputStream(keyStoreLoc);
                ks.load(is, ksPassPhrase);
    ...
    protected SSLEngine setupEngine(String peerHost, int peerPort) {
        ...
        try {
            ...
            for(int i=0; i < supported.length; i++){
            	supportedStr = supportedStr + " " + supported[i];
            }
            logger.log(Level.WARNING, "Default protocols: " + supportedStr);
        }
        ...
        try {
            ...
            logger.log(Level.WARNING, "Default cipherSuits: " + supportedStr);
        }
        return engine;
    }
    ...
    protected void closeSocket(SocketIF socket) {
    	// TODO: Android changes
        //try {
    	SocketAddress remote = socket.getSocket().socket().getRemoteSocketAddress();
    	String remoteStr = (remote == null)?"":remote.toString();
        logger.log(Level.INFO, "Disconnecting remote: {0}", remoteStr);
        //} catch (IOException ioe) {
        ...
        if (socket != null) {
            try {
            	...
            	logger.log(Level.INFO, "NPE trying to close socket", npe);
            	// We need to at least try to cancel the key here
                SelectionKey key = socket.getSocket().keyFor(selector);
                if (key != null) {
                    logger.finest("Cancelling registered key");
