...
public class TCPClient extends AbstractSelector implements SenderIF {
    ...
    private TCPStatusListener statusListener;
    ...
    public TCPClient(String addressString, int port,
            AbstractPacketWorker packetWorker, boolean usingSSL,
            boolean needClientAuth) {
        super(addressString, port, packetWorker, usingSSL, true,
                needClientAuth);
    }
    ...
    protected void initConnection() throws IOException {
    	if(statusListener != null){
    		statusListener.statusChanged(TCPStatusListener.STATUS_CONNECTING);
    	}
    	if(address == null){
    		// we were given a string, create the inetaddress
        ...
        // TODO: Android changes
        channel.socket().setSendBufferSize(PropertiesReader.getSoSndBuf());
        channel.socket().setReceiveBufferSize(PropertiesReader.getSoRcvBuf());
        channel.socket().setKeepAlive(PropertiesReader.getKeepAlive());
        channel.socket().setReuseAddress(PropertiesReader.getReuseAddress());
        channel.socket().setTrafficClass(PropertiesReader.getIPTos());
    ...
    protected void connect(SelectionKey key) {
        ...
        try {
        	...
        	// TODO: Android changes
        	sc.getSocket().socket().setTcpNoDelay(PropertiesReader.getTCPNoDelay());
            //sc.getSocket().setOption(StandardSocketOptions.TCP_NODELAY, PropertiesReader.getTCPNoDelay());
            sc.finishConnect();
        } catch (IOException ioe) {
        	// Notify the listener
        	if(statusListener != null){
        		statusListener.statusChanged(TCPStatusListener.STATUS_CONN_FAILED);
        	}
        ...
        // Notify the listener
        if(statusListener != null){
        	statusListener.statusChanged(TCPStatusListener.STATUS_CONNECTED);
        }
    ...
    public boolean isConnected() {
        Log.d("TCPClient", "client " + this.toString());

        Log.d("TCPClient", "isRunning() " + isRunning());
        Log.d("TCPClient", "this.connected " + this.connected);
    ...
    protected void closeSocket(SocketIF socket) {
    	// Notify the listener
    	if(statusListener != null){
    		statusListener.statusChanged(TCPStatusListener.STATUS_DISCONNECTED);
    	}
